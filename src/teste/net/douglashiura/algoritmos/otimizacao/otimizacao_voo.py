...
pessoas = [('Amanda', 'CWB'), ('Pedro', 'GIG'), ('Marcos', 'POA'), ('Pricila', 'FLN'), ('Jessica', 'CNF'), ('Paulo', 'GYN')]
...
def imprimir_agenda(agenda):
    ...
    for i in range(len(agenda) // 2):
        ...
        id_voo += 1;
        volda = voos[(destino, origem)][agenda[id_voo]]
        print('%10s%10s %5s-%5s R$%3s %5s-%5s R$%3s' % (nome, origem, ida[0], ida[1], ida[2], volda[0], volda[1], volda[2]))
...
def pesquisa_randomica (dominio, funcao_custo):
    melhor_solucao = [random.randint(dominio[i][0], dominio[0][1]) for i in range(len(dominio))] 
    melhor_custo = funcao_custo(melhor_solucao)
